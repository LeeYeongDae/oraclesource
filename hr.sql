...
--SA_MAN인 사람들의 연봉보다 많이 버는 자식들
SELECT e.LAST_NAME, e.JOB_ID, e.SALARY FROM EMPLOYEES e WHERE e.SALARY> ALL(SELECT e.SALARY FROM EMPLOYEES e WHERE e.JOB_ID = 'SA_MAN' );

--커미션 받는 부서와 연봉이 동일한 사람들 last, deptno, salary
SELECT e.LAST_NAME, e.DEPARTMENT_ID, e.SALARY FROM EMPLOYEES e WHERE e.salary > all(SELECT e.SALARY FROM EMPLOYEES e WHERE e.COMMISSION_PCT > 0) ;

--평균 연봉보다 더 버는 사원들중 last_name에 u가 있는 사원들이 근부하는 부서와 같은 부서에 근무하는 사원들의 사번, last, salary
...
--각 부서별 평균 연봉보다 더 받는 동일부서 사원들의 lastname,salary,deptno, 부서의 평균연봉 오름차순
...
AND e.SALARY > p.DEPT_SAL_AVG
ORDER BY p.DEPT_SAL_AVG;
